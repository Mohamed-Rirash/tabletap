defmodule TabletapWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, TabletapWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to their own order history for a user with no staff membership (a customer account, build-plan.md Feature 16), regardless of what was pre-assigned",
      ...
      # calling it is irrelevant, matching what a real request would do.
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> UserAuth.log_in_user(user)
    ...
    test "redirects an owner straight to their venue dashboard", %{conn: conn} do
      %{user: owner} = Tabletap.TenantsFixtures.org_fixture()

      conn = UserAuth.log_in_user(conn, owner)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      TabletapWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        TabletapWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
