defmodule Tabletap.AccountsTest do
  use Tabletap.DataCase

  alias Tabletap.Accounts

  import Tabletap.AccountsFixtures
  alias Tabletap.Accounts.{User, UserToken}
  ...
  describe "login_user_by_magic_link/1" do
    ...
    test "raises when unconfirmed user has password set" do
      user = unconfirmed_user_fixture()
      {1, nil} = Repo.update_all(User, [set: [hashed_password: "hashed"]], skip_org_id: true)
      {encoded_token, _hashed_token} = generate_user_magic_link_token(user)
  ...
  describe "deliver_login_instructions/2" do
    ...
    test "throttles a resend within the cooldown window (design-qa.md Q47)", %{user: user} do
      url_fun = fn token -> "https://example.com/#{token}" end

      assert {:ok, %Swoosh.Email{}} = Accounts.deliver_login_instructions(user, url_fun)

      assert Repo.aggregate(from(t in UserToken, where: t.context == "login"), :count,
               skip_org_id: true
             ) == 1

      # A second send moments later doesn't mint another token or email.
      assert {:ok, :throttled} = Accounts.deliver_login_instructions(user, url_fun)
